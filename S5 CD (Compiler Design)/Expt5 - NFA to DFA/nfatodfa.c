...
int inputs;
// NFA entered as an NxN matrix with the 2D matrices
// stacked for each input
...
void main()
{
    ...
    // Conversion to DFA starts here. A queue is
    // maintained to keep track of the new states 
    //that are found.
    char queue[20][10];
    int front = 0;
    int rear = 0;
    ...
    {
        // Keeps track of new rows of DFA table when 
        //new states are added
        int temp_rows = rows;
        // Contains the new states that are found for an
        // input symbol transition for each state
        char new_states[20];
        ...
        for (int j = 0; j < inputs; j++)
        {
            ...
            // Loop to find the transitions for each input of
            // the current state in the front of the queue
            // i = 1; i += 2: For each sub-state in the current 
            //state - i.e., q0, q1, q2 in q0q1q2
