...
void sortByPRIORITY(int i, int n, int ict)
{
    for(int a=i+1; a<n; a++)
    {
        for(int b=a; b<n; b++)
        {
            if( (p[a].pri>p[b].pri) && (p[a].at<=ict) && (p[b].at<=ict))
                swap(&p[a], &p[b]);
        }
    }
}
...
void nonPremptivePriority(int n)
{
    sortByAT(n);
    int ict = p[0].at;
